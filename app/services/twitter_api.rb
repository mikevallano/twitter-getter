class TwitterApi
  ...
  def get_favorites(username:, count: 200, since_id: '1', max_id: nil)
    client.favorites(username, count: count, since_id: since_id) # TODO, need to account for since & max. the api doesn't want the param unless it's valid
  end
end
